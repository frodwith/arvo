...
++  fitz                                                ::  odor compatibility
  ...
  ?&  ?|  =(0 p.yoz)
      ...
      |-  ?|  =(%$ q.yoz)
              =(%$ q.wux)
              ?&  =((end 3 1 q.yoz) (end 3 1 q.wux))
                  $(q.yoz (rsh 3 1 q.yoz), q.wux (rsh 3 1 q.wux))
              ==
          ==
  ==
::
...
++  ut
  ...
  ++  mull
    ...
    ::
        {$fits *}
      =/  nob/twig  ~(bunt al %herb p.gen)
      =/  dok/twig  [%wing q.gen]
      =/  waz/(pair span span)
        [(play nob) (play(sut dox) nob)]
      =/  syx/(pair axis axis)
        [(doke dok) (doke(sut dox) dok)]
      =/  pov/(pair nock nock)
        [(fish(sut p.waz) p.syx) (fish(sut q.waz) q.syx)]
    ...
    ::
      ...
      =/  hiq/(pair span span)
        =/  fex/(pair span span)
          [(gain p.gen) (gain(sut dox) p.gen)]
        ?:  =(%void p.fex)
          :-  %void
          ?:  =(%void q.fex)
            %void
          (play(sut q.fex) q.gen)
        ?:  =(%void q.fex)
          ~|(%mull-bonk-c !!)
        $(sut p.fex, dox q.fex, gen q.gen)
      =/  ran/(pair span span)
        =/  wux/(pair span span)
          [(lose p.gen) (lose(sut dox) p.gen)]
        ?:  =(%void p.wux)
          :-  %void
          ?:  =(%void q.wux)
            %void
          (play(sut q.wux) r.gen)
        ?:  =(%void q.wux)
          ~|(%mull-bonk-d !!)
    ...
    ::
      ...
      (both bool)
    ::
        {$deep *}  =+($(gen p.gen, gol %noun) (both bool))    ::  XX  =|
    ...
    ::
        {$iron *}
      =+  vat=$(gen p.gen)
      [(wrap(sut p.vat) %iron) (wrap(sut q.vat) %iron)]
    ::
        {$zinc *}
      =+  vat=$(gen p.gen)
      [(wrap(sut p.vat) %zinc) (wrap(sut q.vat) %zinc)]
    ::
        {$lead *}
      =+  vat=$(gen p.gen)
      [(wrap(sut p.vat) %lead) (wrap(sut q.vat) %lead)]
    ::
        {$name *}
      =+  vat=$(gen q.gen)
      [(conk(sut p.vat) p.gen) (conk(sut q.vat) p.gen)]
    ::
        {$burn *}  $(gen p.gen)
    ::
        {$like *}
    ...
    ::
      ...
      =/  zur/(pair span span)
        [(play p.gen) (play(sut dox) p.gen)]
      =/  ret/(pair span span)
        [(cell p.zur p.vos) (cell q.zur q.vos)]
      [(nice p.ret) q.ret]
    ::
        {$lost *}
      ?:  vet
    ...
    ++  doke
      |=  gen/twig  ^-  axis
      =/  fug/(pair span nock)  (mint %noun gen)
      (coke q.fug)
    ::
    ++  nice
      |=  typ/span
